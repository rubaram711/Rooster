...
abstract class SalesInvoiceControllerAbstract extends GetxController {
  ...
  addToOrderLinesInSalesInvoiceList(String index, Widget p);
  removeFromOrderLinesInSalesInvoiceList(String index);
...
class SalesInvoiceController extends SalesInvoiceControllerAbstract {
  ...
  Map<String, Widget> orderLinesSalesInvoiceList = {};
  @override
  resetSalesInvoice() {
      ...
      for (int i = 0; i < unitPriceControllers.length; i++) {
        ...
        if (selectedItemId != '') {
          if (itemUnitPrice.keys.contains(selectedItemId)) {
            ...
            orderLinesSalesInvoiceList.remove('${keys[i]}');
  ...
  @override
  addToOrderLinesInSalesInvoiceList(String index, Widget p) {
    orderLinesSalesInvoiceList[index] = p;
    update();
  }

  @override
  removeFromOrderLinesInSalesInvoiceList(String index) {
    orderLinesSalesInvoiceList.remove(index);
    update();
  }
